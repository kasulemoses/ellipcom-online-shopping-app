...
import com.ellipcom.ellipcom.databinding.FragmentWheelBarrowsBinding
import com.ellipcom.ellipcom.model.ProductInformationModel
import com.google.firebase.firestore.DocumentChange
import com.google.firebase.firestore.FirebaseFirestore


class WheelBarrowsFragment : Fragment() {

    private var _binding: FragmentWheelBarrowsBinding? = null
    ...
    private val productAdapter by lazy { WheelBarrowsAdapter() }
    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View {
        _binding = FragmentWheelBarrowsBinding.inflate(inflater, container, false)


        fireDb = FirebaseFirestore.getInstance()

        binding.wheelBarrowsRV.layoutManager = GridLayoutManager(context, 2)
        productList = ArrayList()
        binding.wheelBarrowsRV.adapter = productAdapter
        attachingDataToRv()
