...
class SmartPhonesAdapter : RecyclerView.Adapter<SmartPhonesAdapter.SmartPhonesViewHolder>() {

    private var oldProductList = ArrayList<ProductInformationModel>()

    class SmartPhonesViewHolder(val productItemBinding: MainProductItemListRecyclerBinding) :
    ...
    fun setData(newList: ArrayList<ProductInformationModel>) {
        val diffUtil = SmartPhonesDiffUtil(
            oldProductList,
            newList
        )
