package com.ellipcom.ellipcom.ui.construction.safetyGears.safetyBoots
...
import com.ellipcom.ellipcom.databinding.FragmentSafetyBootsBinding
...
class SafetyBootsFragment : Fragment() {

    private var _binding: FragmentSafetyBootsBinding? = null
    ...
    private val productAdapter by lazy { SafetyBootsAdapter() }

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        _binding = FragmentSafetyBootsBinding.inflate(inflater, container, false)


        fireDb = FirebaseFirestore.getInstance()

        binding.safetyBootsRV.layoutManager = GridLayoutManager(context, 2)
        productList = ArrayList()
        binding.safetyBootsRV.adapter = productAdapter
        attachingDataToRv()
