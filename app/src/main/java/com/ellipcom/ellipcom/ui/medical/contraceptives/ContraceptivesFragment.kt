package com.ellipcom.ellipcom.ui.medical.contraceptives
...
import com.ellipcom.ellipcom.databinding.FragmentContraceptivesBinding
...
class ContraceptivesFragment : Fragment() {
    ...
    private var _binding: FragmentContraceptivesBinding? = null
    ...
    private val productAdapter by lazy { ContraceptivesAdapter() }

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View {
        _binding = FragmentContraceptivesBinding.inflate(inflater, container, false)


        fireDb = FirebaseFirestore.getInstance()

        binding.contraceptivesRV.layoutManager = GridLayoutManager(context, 2)
        productList = ArrayList()
        binding.contraceptivesRV.adapter = productAdapter
        attachingDataToRv()
